...
final class SearchPlayersPageInteractor {
    ...
    private var playersApplications: [PlayerApplicationResponseModel] = []
    ...
    private func convertResponseToDisplayModel(_ response: [PlayerApplicationResponseModel], completion: @escaping (Result<[PlayersApplicationDisplayModel], Error>) -> Void) {
        DispatchQueue.global().async { [weak self] in
            ...
            Task {
                self.playersApplications = response
...
extension SearchPlayersPageInteractor: SearchPlayersPageInteractorInput {
    ...
    func getImageURLForTeam(with id: Int) -> String? {
        var playersApplication: PlayerApplicationResponseModel?
        playersApplication = self.playersApplications.first(where: {
            $0.id == id
        })
        return playersApplication?.photo
    }
}
